...
supabase/functions/stripe-webhook/index.ts
  ...
  │     └─ sendPaymentConfirmationEmail() → sends via the Resend HTTP API → email delivered to customer
  └─ records event in billing_webhook_events (idempotency guard)
```

**Key invariant:** email failures are isolated in their own `try/catch`. If Resend is unreachable or the credentials are wrong, Stripe still receives a `200` and never retries. The failure is logged to `system_logs` with `event_name = 'payment_confirmation_email_failure'`.
...
| `RESEND_API_KEY` | Authenticates outgoing email via the [Resend](https://resend.com) HTTP API (`re_...`) |
| `EMAIL_FROM` | Sender address; must be on a Resend-verified domain in production |
...
> **You do not need email credentials to develop locally.** Leave `RESEND_API_KEY`/`EMAIL_FROM` blank and the email step is silently skipped — a warning is logged but nothing breaks. Add them only when you want to verify that real emails are being sent.
...
# Email (Resend HTTP API) — optional locally; leave both blank to skip sending
RESEND_API_KEY="re_..."                  # your Resend API key
EMAIL_FROM="onboarding@resend.dev"       # sandbox sender (delivers only to your Resend account email)
```
...
### If you have Resend credentials set
...
### If the email is skipped (no Resend credentials)
...
#### 3. Resend: verify your sending domain and set `EMAIL_FROM`

> **One-time owner task.** This is done once by whoever owns the domain/Resend
> account. A developer who is *given* access to an already-verified Resend account
> skips this entirely — they just use the `RESEND_API_KEY`/`EMAIL_FROM` already set.
...
The script gathers everything from `.deploy/` files, validates them, pushes to both Supabase and Vercel, then shreds the files. See [prod_setup.md](../how_to/prod_setup.md) for the full bootstrap procedure.
...
- [Production Setup & Deploy](../how_to/prod_setup.md) — complete bootstrap procedure with credential management
