# How to Load Test and Optimize Performance

This guide covers database performance tuning, connection pooling, and load
testing for GrantTrail. Follow these steps when you need to validate that the
application handles high-concurrency traffic.
...
For dashboard sorting and timeline queries under high load, add the following
composite indexes. Run these in the Supabase SQL Editor:
...
Direct PostgreSQL connections will exhaust database connection limits under high
concurrent load. Supabase uses Supavisor for connection pooling.
